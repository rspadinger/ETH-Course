//SPDX-License-Identifier: MIT
pragma solidity 0.8.20;

contract HelloWorld {
    //TODO add a public state variable (message) of type string

    //TODO create an event (UpdateMessage) that logs the address of the caller, the old string value (oldMessage) and the modified string value

    //TODO create a constructor that sets an initial value for the state variable declared above
    
    //TODO create a function (updateMessage) that modifies the state variable and emits the "UpdateMessage" event    
}
